...
const memberRows = (userId: string, items: FamilyMember[]) => items.map((item) => ({ user_id: userId, id: item.id, auth_user_id: item.authUserId ?? null, name: item.name, role: item.role ?? null, archived: item.archived ?? false, income: item.income ?? null, expense: item.expense ?? null, member_since: item.createdAt?.slice(0, 10) ?? null }))
export const transactionRowForSupabase = (userId: string, item: Transaction) => ({ user_id: userId, id: item.id, type: item.type, amount: item.amount, category: item.category, description: item.description, occurred_on: item.date, account_id: item.accountId, family_member_id: item.familyMemberId ?? null, expense_scope: item.expenseScope ?? null, attachments: item.attachments ?? [], receipt_attachment: item.receiptAttachment ?? null, source_created_at: item.createdAt ?? null, source_updated_at: item.updatedAt ?? null })
const transactionRows = (userId: string, items: Transaction[]) => items.map((item) => transactionRowForSupabase(userId, item))
...
const accountFromRow = (row: Row): Account => ({ id: String(row.id), name: String(row.name), icon: String(row.icon), type: optionalString(row.type), openingBalance: row.opening_balance === null ? undefined : number(row.opening_balance), balance: row.legacy_balance === null ? undefined : number(row.legacy_balance), description: optionalString(row.description) })
const familyMemberFromRow = (row: Row): FamilyMember => ({ id: String(row.id), authUserId: optionalString(row.auth_user_id), name: String(row.name), role: optionalString(row.role), archived: Boolean(row.archived), income: row.income === null ? undefined : number(row.income), expense: row.expense === null ? undefined : number(row.expense), createdAt: optionalString(row.member_since) })

export async function fetchTransactionFormOptions(userId: string): Promise<{ accounts: Account[]; familyMembers: FamilyMember[] }> {
  ...
  return {
    accounts: ((accountsResult.data ?? []) as Row[]).map(accountFromRow),
    familyMembers: ((membersResult.data ?? []) as Row[]).map(familyMemberFromRow),
  }
}
...
export async function fetchFinanceState(userId: string, fallbackPreferences: AppPreferences): Promise<FinanceStateData> {
  ...
  return {
    ...
    familyMembers: members.map(familyMemberFromRow),
