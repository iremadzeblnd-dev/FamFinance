...
import { Bell, Database, Download, FileDown, Plus, ShieldCheck, Tags, Trash2, Upload, X } from 'lucide-react'
import { useFinance } from '../state/FinanceContext'
import { translate } from '../i18n/translations'

const labels = {
  ka: ['შეტყობინებები','ვალები და გადახდები','ბიუჯეტის ლიმიტები','დანაზოგის მიზნები','დაგეგმილი შეხსენებები','კატეგორიების მართვა','შემოსავლის კატეგორიები','ხარჯის კატეგორიები','ახალი კატეგორია','დამატება','მონაცემების მართვა','სარეზერვო ასლი','აღდგენა','CSV ექსპორტი','უსაფრთხოება','შეტყობინებებში თანხების დამალვა','ყველა მონაცემის წაშლა','ნამდვილად გსურთ ყველა ფინანსური მონაცემის წაშლა? ამ მოქმედების გაუქმება შეუძლებელია.','აღდგენა ჩაანაცვლებს მიმდინარე მონაცემებს. გაგრძელება?','მონაცემები აღდგენილია.','ფაილი არასწორი ან დაზიანებულია.','შეტყობინებების ჩართვა','ძველი ლოკალური მონაცემების იმპორტი','ძველი მონაცემები ამ ანგარიშში დაემატება. გაგრძელება?','ძველი მონაცემები წარმატებით დაემატა.','იმპორტი ვერ მოხერხდა. ძველი მონაცემები არ წაშლილა.'],
  en: ['Notifications','Debts and payments','Budget limits','Savings goals','Scheduled reminders','Category management','Income categories','Expense categories','New category','Add','Data management','Backup','Restore','Export CSV','Security','Hide amounts in notifications','Clear all data','Delete all financial data? This cannot be undone.','Restoring will replace current data. Continue?','Data restored successfully.','The file is invalid or damaged.','Enable notifications','Import legacy local data','Legacy data will be added to this account. Continue?','Legacy data imported successfully.','Import failed. The legacy data was not deleted.'],
  ru: ['Уведомления','Долги и платежи','Лимиты бюджета','Цели накоплений','Запланированные напоминания','Управление категориями','Категории доходов','Категории расходов','Новая категория','Добавить','Управление данными','Резервная копия','Восстановить','Экспорт CSV','Безопасность','Скрывать суммы в уведомлениях','Удалить все данные','Удалить все финансовые данные? Это действие нельзя отменить.','Восстановление заменит текущие данные. Продолжить?','Данные восстановлены.','Файл недействителен или повреждён.','Включить уведомления','Импорт старых локальных данных','Старые данные будут добавлены в этот аккаунт. Продолжить?','Старые данные успешно импортированы.','Импорт не выполнен. Старые данные не удалены.'],
  tr: ['Bildirimler','Borçlar ve ödemeler','Bütçe limitleri','Tasarruf hedefleri','Planlanmış hatırlatıcılar','Kategori yönetimi','Gelir kategorileri','Gider kategorileri','Yeni kategori','Ekle','Veri yönetimi','Yedekle','Geri yükle','CSV dışa aktar','Güvenlik','Bildirimlerde tutarları gizle','Tüm verileri temizle','Tüm finansal veriler silinsin mi? Bu işlem geri alınamaz.','Geri yükleme mevcut verilerin yerini alacak. Devam edilsin mi?','Veriler geri yüklendi.','Dosya geçersiz veya bozuk.','Bildirimleri etkinleştir','Eski yerel verileri içe aktar','Eski veriler bu hesaba eklenecek. Devam edilsin mi?','Eski veriler başarıyla içe aktarıldı.','İçe aktarma başarısız oldu. Eski veriler silinmedi.'],
} as const
...
export function SettingsPage() {
  ...
  const { preferences, updatePreferences, restoreFinanceData, clearAllData, legacyImportAvailable, importLegacyData } = finance
  ...
  const exportCsv = () => {
    const rows: unknown[][] = [['date','type','amount','currency','category','description','account','familyMember'], ...finance.transactions.map((item) => [item.date,item.type,item.amount,'GEL',item.category,item.description,finance.accounts.find((a) => a.id === item.accountId)?.name ?? '',finance.familyMembers.find((m) => m.id === item.familyMemberId)?.name ?? ''])]
    saveFile(`\uFEFF${rows.map((row) => row.map(csv).join(',')).join('\n')}`, 'text/csv;charset=utf-8', `family-finances-${new Date().toISOString().slice(0,10)}.csv`)
  }
  const restore = async (event: ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; event.target.value=''; if (!file || !confirm(l[18])) return; try { setStatus(restoreFinanceData(JSON.parse(await file.text())) ? l[19] : l[20]) } catch { setStatus(l[20]) } }
  const importLegacy = async () => { if (!confirm(l[23])) return; setStatus(''); setStatus(await importLegacyData() ? l[24] : l[25]) }
  ...
  return <div className="space-y-6"><div><p className="text-sm uppercase tracking-[0.2em] text-slate-500">{t('settings.eyebrow')}</p><h1 className="mt-2 text-3xl font-bold text-slate-900">{t('settings.title')}</h1></div><div className="grid gap-6 xl:grid-cols-2">
    <section className={card}><Heading icon={<Bell className="h-5 w-5" />} title={l[0]} /><Check label={l[21]} checked={preferences.notifications.enabled} action={() => toggleNotification('enabled')} />{(['debts','budgets','savings','reminders'] as const).map((key,index) => <Check key={key} label={l[index+1]} checked={preferences.notifications[key]} disabled={!preferences.notifications.enabled} action={() => toggleNotification(key)} />)}</section>
    <section className={card}><Heading icon={<ShieldCheck className="h-5 w-5" />} title={l[14]} /><Check label={l[15]} checked={preferences.hideNotificationAmounts} action={() => updatePreferences({hideNotificationAmounts:!preferences.hideNotificationAmounts})} /></section>
    <section className={`${card} xl:col-span-2`}><Heading icon={<Tags className="h-5 w-5" />} title={l[5]} /><div className="grid gap-6 lg:grid-cols-2"><CategoryEditor title={l[6]} items={preferences.incomeCategories} value={names.income} placeholder={l[8]} addLabel={l[9]} onValueChange={(value) => setNames((current) => ({ ...current, income: value }))} onAdd={() => changeCategories('income')} onRemove={(value) => changeCategories('income', value)} /><CategoryEditor title={l[7]} items={preferences.expenseCategories} value={names.expense} placeholder={l[8]} addLabel={l[9]} onValueChange={(value) => setNames((current) => ({ ...current, expense: value }))} onAdd={() => changeCategories('expense')} onRemove={(value) => changeCategories('expense', value)} /></div></section>
    <section className={`${card} xl:col-span-2`}><Heading icon={<Database className="h-5 w-5" />} title={l[10]} /><input ref={fileRef} type="file" accept="application/json,.json" onChange={(e)=>void restore(e)} className="hidden" /><div className="flex flex-wrap gap-3"><button onClick={backup} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 font-medium text-slate-700"><Download className="h-4 w-4" />{l[11]}</button><button onClick={()=>fileRef.current?.click()} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 font-medium text-slate-700"><Upload className="h-4 w-4" />{l[12]}</button><button onClick={exportCsv} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 font-medium text-slate-700"><FileDown className="h-4 w-4" />{l[13]}</button>{legacyImportAvailable?<button type="button" onClick={()=>void importLegacy()} className="inline-flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 font-medium text-amber-800"><Upload className="h-4 w-4" />{l[22]}</button>:null}</div>{status?<p className="mt-3 text-sm text-slate-600" role="status">{status}</p>:null}<div className="mt-6 border-t border-slate-200 pt-5"><h3 className="font-semibold text-rose-700">{l[16]}</h3><button type="button" onClick={()=>{if(confirm(l[17])){clearAllData();setStatus(l[16])}}} className="mt-3 inline-flex items-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 font-medium text-rose-700"><Trash2 className="h-4 w-4" />{l[16]}</button></div></section>
  </div></div>
}
